# Main Gem Rail

### **Main Gem Rail Tab**

Now, we’ll configure the **Main Gem Rail Tab**.

Here are the parameters:

* **Width**: 4.4 mm
* **Height**: 5 mm
* **Thread Diameter**: 1.6 mm
* **Move in Z**: -4.8 mm
* **Bend Height**: 0.3 mm
* **Bend Rotation**: 0

![](https://www.rhinoartisan.com/wp-content/uploads/2024/11/Main-gem-Rail-1024x476.png)

The **Bend Height** is especially important because it ensures the rail follows the curve of the ring’s inner side. This gives our design a seamless, comfortable fit.<br>

### **Video Lesson**

{% embed url="https://www.rhinoartisan.com/wp-content/uploads/2024/11/Lesson-4-1.mp4" %}

**Next Step**\
Configuring the Shank – The Ring Band<br>
